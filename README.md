...
| 📋 **剪贴板秒转 Word** | 双击图标，剪贴板里的 Markdown 内容直接变成 Word 文档（默认保存在桌面） |
| 📑 **标题完美映射** | Word 大纲级别 ⇄ Markdown `#`~`######`，各级标题精准保留 |
| 📊 **表格双向保留** | Word 表格 ↔ Markdown 表格，格式无损 |
| 🖼️ **图片自动提取** | Word→MD 时图片自动导出到本地文件夹 |
| 📝 **内联格式** | 粗体、斜体、删除线、行内代码、超链接 — 全部保留 |
| 📐 **Word 格式可控** | MD→Word 严格按模板样式导出，内置通用模板，支持自定义模板，多模版切换 |
| 🔔 **原生通知** | Windows 10/11 系统通知，点通知直接打开转换后的文件 |
...
设想这个场景：AI给你输出一段标题样式规整的 Markdown 格式的文字，想把它们整理成规整的 Word 文档。
...
| 📄 **办公人员** | 图文并茂的大体积Word想发给ai，图片没有必要一并传 | Word→MD 一键提取带标题样式的文字，体积仅为纯文本大小 |
...
| 🔤 **.doc 需 Word 支持** | 低 | `.doc` 需本机安装 Word，通过 COM 后台转为 `.docx` 再转换。未安装 Word 则无法处理，建议先另存为 `.docx` |
